...
import { addLiquidity, calculateCDAmount } from "../utils/addLiquidity";
import {
  getCDTokensBalance,
  getEtherBalance,
  getLPTokensBalance,
...
export default function Home() {
  ...
  const _getTokensAfterRemove = async (_removeLPTokens) => {
    try {
      ...
      const { ethToRemove, cdTokensToRemove } = await getTokensAfterRemove(
        provider,
        removeLPTokenWei,
        _ethBalance,
        cryptoDevTokenReserve
      );
      setRemoveEther(ethToRemove);
      setRemoveCD(cdTokensToRemove);
    } catch (err) {
      console.error(err);
    }
  };
              ...
              <div>
                <input
                  ...
                  onChange={async (e) => {
                    ...
                    const _addCDTokens = await calculateCDAmount(
                      e.target.value || "0",
